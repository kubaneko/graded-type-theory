...
-- Universe type
record _⊩⟨_⟩U (Γ : Con Term ℓ) (l : TypeLevel): Set a where
  constructor Uᵣ
  field
    l′ : TypeLevel
    l< : l′ < l
...
_⊩⟨_⟩U≡_ : (Γ : Con Term ℓ) → TypeLevel → (B : Term ℓ) → Set a
Γ ⊩⟨ _ ⟩U≡ B = B PE.≡ U -- Note lack of reduction

mutual
  -- Universe term
  record _⊩⟨_⟩U_∷U/_ {l′} (Γ : Con Term ℓ) (l : TypeLevel) (t : Term ℓ) (l< : l′ < l) : Set a where
    inductive
    constructor Uₜ
    eta-equality
    field
  ...
  record _⊩⟨_⟩U_≡_∷U/_ {l′} (Γ : Con Term ℓ) (l : TypeLevel) (t u : Term ℓ) (l< : l′ < l) : Set a where
    inductive
    constructor Uₜ₌
    eta-equality
    field
  ...
  record _⊩⟨_⟩Id_ (Γ : Con Term ℓ) (l : TypeLevel) (A : Term ℓ) : Set a where
    inductive
    constructor Idᵣ
    eta-equality
    field
  ...
  record _⊩⟨_⟩Id_≡_/_
    (Γ : Con Term ℓ) (l : TypeLevel) (A B : Term ℓ) (⊩A : Γ ⊩⟨ l ⟩Id A) : Set a where
    inductive
    constructor Id₌
    eta-equality
    open _⊩⟨_⟩Id_ ⊩A
    field
      ...
      Ty≡Ty′        : Γ ⊩⟨ l ⟩ Ty ≡ Ty′ / ⊩Ty
      lhs≡lhs′      : Γ ⊩⟨ l ⟩ lhs ≡ lhs′ ∷ Ty / ⊩Ty
      rhs≡rhs′      : Γ ⊩⟨ l ⟩ rhs ≡ rhs′ ∷ Ty / ⊩Ty
      ...
      lhs≡rhs→lhs′≡rhs′ : Γ ⊩⟨ l ⟩ lhs  ≡ rhs  ∷ Ty / ⊩Ty →
                          Γ ⊩⟨ l ⟩ lhs′ ≡ rhs′ ∷ Ty / ⊩Ty
      lhs′≡rhs′→lhs≡rhs : Γ ⊩⟨ l ⟩ lhs′ ≡ rhs′ ∷ Ty / ⊩Ty →
                          Γ ⊩⟨ l ⟩ lhs  ≡ rhs  ∷ Ty / ⊩Ty

  -- Well-formed identity terms.
  _⊩⟨_⟩Id_∷_/_ : (Γ : Con Term ℓ) (l : TypeLevel) (t A : Term ℓ)
    → Γ ⊩⟨ l ⟩Id A → Set a
  Γ ⊩⟨ l ⟩Id t ∷ A / ⊩A =
    ∃ λ u →
    Γ ⊢ t :⇒*: u ∷ Id Ty lhs rhs ×
    ...
    case u-id of λ where
      ...
      rflₙ   → Γ ⊩⟨ l ⟩ lhs ≡ rhs ∷ Ty / ⊩Ty
    where
    open _⊩⟨_⟩Id_ ⊩A

  -- Well-fₗormed identity term equality.
  _⊩⟨_⟩Id_≡_∷_/_ : (Γ : Con Term ℓ) (l : TypeLevel) (t u A : Term ℓ)
    → Γ ⊩⟨ l ⟩Id A → Set a
  Γ ⊩⟨ l ⟩Id t ≡ u ∷ A / ⊩A =
    ∃₂ λ t′ u′ →
    Γ ⊢ t :⇒*: t′ ∷ Id Ty lhs rhs ×
  ...
  data _⊩⟨_⟩_ (Γ : Con Term ℓ) (l : TypeLevel) : Term ℓ → Set a where
    Uᵣ  : Γ ⊩⟨ l ⟩U → Γ ⊩⟨ l ⟩ U
    ℕᵣ  : ∀ {A} → Γ ⊩ℕ A → Γ ⊩⟨ l ⟩ A
    Emptyᵣ : ∀ {A} → Γ ⊩Empty A → Γ ⊩⟨ l ⟩ A
    Unitᵣ : ∀ {A} {s : Strength} → Γ ⊩Unit⟨ s ⟩ A → Γ ⊩⟨ l ⟩ A
    ne  : ∀ {A} → Γ ⊩ne A → Γ ⊩⟨ l ⟩ A
    -- Bᵣ  : ∀ {A} W → Γ ⊩ₗB⟨ W ⟩ A → Γ ⊩ₗ A
    Idᵣ : ∀ {A} → Γ ⊩⟨ l ⟩Id A → Γ ⊩⟨ l ⟩ A
    emb : ∀ {A} l′ → l′ < l → Γ ⊩⟨ l′ ⟩ A → Γ ⊩⟨ l ⟩ A

  _⊩⟨_⟩_≡_/_ : (Γ : Con Term ℓ) (l : TypeLevel) (A B : Term ℓ) → Γ ⊩⟨ l ⟩ A → Set a
  Γ ⊩⟨ l ⟩ A ≡ B / Uᵣ UA = Γ ⊩⟨ l ⟩U≡ B
  Γ ⊩⟨ l ⟩ A ≡ B / ℕᵣ D = Γ ⊩ℕ A ≡ B
  Γ ⊩⟨ l ⟩ A ≡ B / Emptyᵣ D = Γ ⊩Empty A ≡ B
  Γ ⊩⟨ l ⟩ A ≡ B / Unitᵣ {s = s} D = Γ ⊩Unit⟨ s ⟩ A ≡ B
  Γ ⊩⟨ l ⟩ A ≡ B / ne neA = Γ ⊩ne A ≡ B / neA
  -- Γ ⊩⟨ l ⟩ A ≡ B / Bᵣ W BA = Γ ⊩ₗB⟨ W ⟩ A ≡ B / BA
  Γ ⊩⟨ l ⟩ A ≡ B / Idᵣ ⊩A = Γ ⊩⟨ l ⟩Id A ≡ B / ⊩A
  Γ ⊩⟨ l ⟩ A ≡ B / emb l′ _ [A] = Γ ⊩⟨ l′ ⟩ A ≡ B / [A]

  _⊩⟨_⟩_∷_/_ : (Γ : Con Term ℓ) (l : TypeLevel) (t A : Term ℓ) → Γ ⊩⟨ l ⟩ A → Set a
  Γ ⊩⟨ l ⟩ t ∷ .U / Uᵣ (Uᵣ _ l< _) = Γ ⊩⟨ l ⟩U t ∷U/ l<
  Γ ⊩⟨ l ⟩ t ∷ A / ℕᵣ D = Γ ⊩ℕ t ∷ℕ
  Γ ⊩⟨ l ⟩ t ∷ A / Emptyᵣ D = Γ ⊩Empty t ∷Empty
  Γ ⊩⟨ l ⟩ t ∷ A / Unitᵣ {s = s} D = Γ ⊩Unit⟨ s ⟩ t ∷Unit
  Γ ⊩⟨ l ⟩ t ∷ A / ne neA = Γ ⊩ne t ∷ A / neA
  -- Γ ⊩⟨ l ⟩ t ∷ A / Bᵣ BΠ! ΠA  = Γ ⊩ₗΠ t ∷ A / ΠA
  -- Γ ⊩⟨ l ⟩ t ∷ A / Bᵣ BΣ! ΣA  = Γ ⊩ₗΣ t ∷ A / ΣA
  Γ ⊩⟨ l ⟩ t ∷ A / Idᵣ ⊩A = Γ ⊩⟨ l ⟩Id t ∷ A / ⊩A
  Γ ⊩⟨ l ⟩ t ∷ A / emb l′ _ [A] = Γ ⊩⟨ l′ ⟩ t ∷ A / [A]

  _⊩⟨_⟩_≡_∷_/_ : (Γ : Con Term ℓ) (l : TypeLevel) (t u A : Term ℓ) → Γ ⊩⟨ l ⟩ A → Set a
  Γ ⊩⟨ l ⟩ t ≡ u ∷ .U / Uᵣ (Uᵣ _ l< _) = Γ ⊩⟨ l ⟩U t ≡ u ∷U/ l<
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / ℕᵣ D = Γ ⊩ℕ t ≡ u ∷ℕ
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / Emptyᵣ D = Γ ⊩Empty t ≡ u ∷Empty
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / Unitᵣ {s = s} D = Γ ⊩Unit⟨ s ⟩ t ≡ u ∷Unit
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / ne neA = Γ ⊩ne t ≡ u ∷ A / neA
-- Γ ⊩⟨ l ⟩ t ≡ u ∷ A / Bᵣ BΠ! ΠA = Γ ⊩ₗΠ t ≡ u ∷ A / ΠA
-- Γ ⊩⟨ l ⟩ t ≡ u ∷ A / Bᵣ BΣ! ΣA  = Γ ⊩ₗΣ t ≡ u ∷ A / ΣA
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / Idᵣ ⊩A = Γ ⊩⟨ l ⟩Id t ≡ u ∷ A / ⊩A
  Γ ⊩⟨ l ⟩ t ≡ u ∷ A / emb l′ _ [A] = Γ ⊩⟨ l′ ⟩ t ≡ u ∷ A / [A]
